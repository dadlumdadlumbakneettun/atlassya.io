...
const atlasSignCanvas=document.createElement('canvas');atlasSignCanvas.width=512;atlasSignCanvas.height=128;const atlasSignCtx=atlasSignCanvas.getContext('2d');atlasSignCtx.fillStyle='#1a0a02';atlasSignCtx.fillRect(0,0,512,128);atlasSignCtx.strokeStyle='#8B6914';atlasSignCtx.lineWidth=6;atlasSignCtx.strokeRect(6,6,500,116);atlasSignCtx.fillStyle='#FFD700';atlasSignCtx.font="bold 54px serif";atlasSignCtx.textAlign='center';atlasSignCtx.textBaseline='middle';atlasSignCtx.fillText("Atlas'ın Odası",256,64);const atlasSignTex=new THREE.CanvasTexture(atlasSignCanvas);
const noteCanvas=document.createElement('canvas');noteCanvas.width=256;noteCanvas.height=160;const noteCtx=noteCanvas.getContext('2d');noteCtx.fillStyle='#f5f0dc';noteCtx.fillRect(0,0,256,160);noteCtx.save();noteCtx.translate(128,80);noteCtx.rotate(-0.08);noteCtx.fillStyle='rgba(180,160,120,0.18)';noteCtx.fillRect(-128,-80,256,160);noteCtx.restore();noteCtx.strokeStyle='rgba(100,80,40,0.12)';noteCtx.lineWidth=1;for(let i=0;i<10;i++){noteCtx.beginPath();noteCtx.moveTo(Math.random()*256,0);noteCtx.lineTo(Math.random()*256,160);noteCtx.stroke();}for(let i=0;i<6;i++){noteCtx.beginPath();noteCtx.moveTo(0,Math.random()*160);noteCtx.lineTo(256,Math.random()*160);noteCtx.stroke();}noteCtx.fillStyle='rgba(120,100,60,0.13)';noteCtx.beginPath();noteCtx.moveTo(200,0);noteCtx.lineTo(256,0);noteCtx.lineTo(256,56);noteCtx.closePath();noteCtx.fill();noteCtx.fillStyle='#1a1a1a';noteCtx.font="bold 36px monospace";noteCtx.textAlign='center';noteCtx.textBaseline='middle';noteCtx.save();noteCtx.translate(128,80);noteCtx.rotate(-0.07);noteCtx.fillText("No service",0,0);noteCtx.restore();const noteTex=new THREE.CanvasTexture(noteCanvas);
const atlasDoor=new THREE.Mesh(dGeo,new THREE.MeshStandardMaterial({map:doorTex,bumpMap:doorTex,bumpScale:0.04,roughness:0.6}));atlasDoor.position.set(-2.35,1.6,-7);atlasDoor.rotation.y=Math.PI/2;scene.add(atlasDoor);addKnob(atlasDoor);
...
noteMesh.position.set(-2.22,0.98,-6.72);
noteMesh.rotation.set(0.08, Math.PI/2, 0.28);
scene.add(noteMesh);
}
...
hwBtnGroup.position.set(cx+5.88,0.55,cz-1.5);
...
const bowR=new THREE.Mesh(new THREE.ConeGeometry(0.04,0.06,16),bowMat);
    ...
    const cwTvCasing=new THREE.Mesh(new THREE.BoxGeometry(0.22,2.3,4.3),new THREE.MeshStandardMaterial({color:0xdd44aa,roughness:0.4,metalness:0.3}));
    cwTvCasing.position.set(cx+5.89,2.8,cz+2.5);scene.add(cwTvCasing);
    ...
    cwScreen2.position.set(cx+5.77,2.8,cz+2.5);cwScreen2.rotation.y=-Math.PI/2;scene.add(cwScreen2);
